...
public class HorizontalProgressWheelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.d("QuyDD", "onDraw mTotalScrollDistance " + mTotalScrollDistance + ", linesCount1 " + linesCount + ", deltaX " + deltaX + ", spacingItem " + spacingItem);
        ...
        for (int i = 0; i < max; i++) {
            if (i < max - 4) {
                ...
                mProgressLinePaint.setAlpha((int) (255 * ((max - (i + 1)) / (float) (max - 4))));
            }
...
//            // Thêm từ giữa sang trái
    ...
    private void onScrollEvent(MotionEvent event, float distance) {
        ...
        Log.d("QuyDD", "onScrollEvent = " + mTotalScrollDistance + ", distance = " + distance);
        postInvalidate();
        mLastTouchedPosition = event.getX();
        if (mScrollingListener != null) {
